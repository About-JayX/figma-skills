...
`codegen_pipeline.mjs` 和 `generate_skeleton.mjs` 都会调 `lib/render_node.mjs:collectFontFamilyWeights` 遍历整棵 bridge tree，按实际用到的 `{family, weight, italic}` 组合自动注入 `<link rel="stylesheet" href="https://fonts.googleapis.com/css2?...">` 到 `<head>`。`emit_css.mjs` 同时在 `App.css` 顶部输出 `@import` 作为兜底。

**没有任何字体白名单**——只要 family 是合法的 Google Fonts 名字就会被请求；如果 family 不在 Google Fonts 上，请求会 404，浏览器降级到 fallback。**不要**给 family 加白名单门控（曾经踩过：`Orbitron`/`Advent Pro` 不在白名单 → 静默 fallback 到 sans-serif → SSIM 永远不达标）。

### 手工 fallback 时

如果 Step 3a generator 没用、自己写 HTML/JSX：
...
`:wght@` 后列出所有用到的 weight（查 `text.segments[].fontWeight`），分号分隔，`display=swap` 避免首帧空白。Italic 用 `family=<Name>:ital,wght@0,<w>;1,<w>` 格式同时请求 normal + italic。
