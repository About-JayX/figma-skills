...
## Styled text segments（富文本）

TEXT 节点有 `text.segments[]`（plugin 通过 `getStyledTextSegments` 提取），每段含：

- `characters`, `start`, `end`（字符切片定位）
- `fontName`, `fontSize`, `fontWeight`, `fontStyle`, `fills`（per-段样式）
- `lineHeight`, `letterSpacing`, `textDecoration*`, `textCase`
- `hyperlink`, `boundVariables`, `textStyleOverrides`, `openTypeFeatures`

**规则**：一个 TEXT 节点内有 ≥2 段不同样式时，必须拆成 `<span>` 链（每段一个 span，使用该段的 fontSize/fills 等），**不能只取第一段样式整段渲染**。官方 Figma MCP 在这个场景会直接出错，本 Skill 需要把富文本 100% 还原。

含 `hyperlink` 的段落包在 `<a>` 里。`boundVariables` 段按 token-extraction.md 的规则输出 `var(--xxx)`。

## 布局盒（resolvedLayout）

Figma 节点 layout 里同时有 `absoluteBoundingBox` 和 `absoluteRenderBounds`：

| 字段 | 含义 | CSS 对应 |
|------|------|---------|
| `absoluteBoundingBox` | stroke 含在内（按 strokeAlign），不含 effects/shadow | CSS `border-box`（`box-sizing: border-box` + `padding` + `border-width`） |
| `absoluteRenderBounds` | 含所有可见像素（stroke + drop-shadow + blur 溢出） | 父容器若 `overflow: visible`，实际绘制边界 |
| `paddingTop/Right/Bottom/Left` | 内部 padding | CSS `padding-*` |
| `strokeWeight`（或 `strokeTopWeight` 等） | 描边粗细 | CSS `border-width` |
| `strokeAlign`（`INSIDE` / `CENTER` / `OUTSIDE`） | 描边相对节点边线的位置 | 无直接对应，需要调 box-sizing/outline |

**规则**：嵌套 FILL 或多层 stroke 导致 1-2px 偏差时，用 `absoluteBoundingBox.width/height` 作为 borderBox 的事实值，不要用 `layout.width/height`（那是 Figma 内部声明值，在 FILL 链路上可能已被父容器重算）。stroke `INSIDE` 时用 `box-sizing: border-box`，`OUTSIDE` 时通常用 `outline`，`CENTER` 时按需要近似。
